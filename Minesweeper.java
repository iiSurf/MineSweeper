...
/**
 * This minesweeper algorithm reads at least one minsweeper fields from System.in or
 * command line argument source and prints each field with the number of adjacent mines to
...
public final class Minesweeper {
    ...
    /**
     * Program entry point. Reads input either from a file specified as a single command-line
     * argument or from standard input. The input consists of one or more Minesweeper fields.
     * Each field begins with two integers n and m (rows and columns). A pair of zeros (0 0)
     * terminates the input. For each field the program reads the board and prints the field
     * with numbers indicating how many adjacent mines each non-mine cell has.
     *
     * @param theArgs command-line arguments; if one argument is provided it is treated as the
     *                path to an input file to read instead of System.in
     * @throws FileNotFoundException if the file specified in theArgs[0] cannot be opened
     */
    ...
    /**
     * Reads a Minesweeper board from the provided Scanner. The method expects exactly n lines
     * to follow in the scanner, each containing m characters representing the board row.
     * Each character is copied into a newly allocated 2D char array which is returned.
     *
     * @param theScan the Scanner to read board lines from; must be positioned at the first board line
    ...
    /**
     * Processes a single Minesweeper field: reads the board from the scanner, prints the
     * field header "Field #X:" and then prints the board where each mine ('*') is printed
    ...
    /**
     * Counts the number of mines adjacent to the specified cell in theBoard. Adjacent cells
     * include the eight surrounding positions (horizontal, vertical, and diagonal) when they
    ...
    private static int neighbors(final char[][] theBoard, final int theRow, final int theColumn) {
        ...
        if (theRow + 1 < theBoard.length && theColumn + 1 < theBoard[theRow].length && theBoard [theRow + 1][theColumn + 1] == MINE) {
            nums = nums + 1;
        }
        return nums;
    }
